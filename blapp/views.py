...
def saved(request):
    ...
    user_object = User.objects.get(username=request.user.username)
    user_profile = Profile.objects.get(user=user_object)
    return render(request,'saved.html',{'saved_posts': saved_posts,'user_profile':user_profile})
...
def save_post(request, post_id):
    ...
    user = request.user
    saved_post = SavedPost.objects.filter(user=user, original_post=original_post).first()

    if saved_post:
        # If the post is already saved, delete the saved post
        saved_post.delete()
    else:
        # If the post is not saved, create a new SavedPost instance
        saved_post = SavedPost(user=user, original_post=original_post)
        saved_post.save()

    # Redirect the user back to the 'saved' view, not 'post_detail' view
...
def following_page(request):
    ...
    return render(request,'following.html',{'user_profile':user_profile, 'posts':feed_list,})


@login_required(login_url='signin_page')
def profile_page(request):
    user_object = request.user  # Use the logged-in user's object
    user_profile = Profile.objects.get(user=user_object)
